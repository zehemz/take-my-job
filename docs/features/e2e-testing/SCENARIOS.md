...
| ID | Scenario | Status |
|----|----------|--------|
| E2E-RBAC-001 | `GET /api/admin/users` without admin role → 401 Unauthorized | ✅ Implemented |
| E2E-RBAC-002 | `GET /api/admin/groups` without admin role → 401 Unauthorized | ✅ Implemented |
| E2E-RBAC-003 | Admin can create a user via `POST /api/admin/users` with valid GitHub username | ✅ Implemented |
| E2E-RBAC-004 | Admin can create a group with agent roles + environment access | ✅ Implemented |
| E2E-RBAC-005 | Admin can add a user to a group via `POST /api/admin/groups/:id/members` | ✅ Implemented |
...
| E2E-RBAC-015 | Cannot delete the last admin user → 400 | ✅ Implemented |
| E2E-RBAC-016 | Cannot demote the last admin → 400 | 🟡 Planned |
| E2E-RBAC-017 | Navigate to `/access` as admin → page renders Users/Groups tabs | ✅ Implemented |
| E2E-RBAC-018 | Navigate to `/access` as non-admin → redirected to `/` | 🟡 Planned |
| E2E-RBAC-019 | Wildcard `*` agent role grants access to all agent roles | ✅ Implemented |
| E2E-RBAC-020 | Wildcard `*` environment grants access to all environments | ✅ Implemented |
| E2E-RBAC-021 | "Access" nav link visible only to admin users | ✅ Implemented |

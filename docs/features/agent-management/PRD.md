...
model/version they run, or which Anthropic environment they belong to — without querying the database
directly. This becomes a maintenance and debugging bottleneck as the agent roster grows.

## Goal

Provide a first-class **Agent Management** page where operators can view and manage the full set of
configured agents. The initial scope covers **read-only listing** (the "R" in CRUD). Create / Update /
Delete operations are explicitly deferred and will be specced separately.
...
- Deleting or deactivating agents from the UI.
...
2. Each row displays: role, Anthropic agent ID, agent version, environment ID, and creation date.
3. The list loads correctly when there are zero configs (empty state shown).
4. No sensitive secrets are exposed — agent IDs and environment IDs are display-only references, not
   credentials.
...
- Delete: retire an agent config and optionally archive its run history.

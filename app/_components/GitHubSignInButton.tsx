...
export default function GitHubSignInButton({ callbackUrl }: Props) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className={`w-full flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-100 border border-zinc-700 rounded-lg px-4 py-3 text-sm font-medium transition-colors duration-150 cursor-pointer${loading ? ' opacity-75 pointer-events-none' : ''}`}
    >
      {loading ? (
        /* Spinner */
        <svg
